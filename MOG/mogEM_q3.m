...
function [p,mu,vary,logProbX] = mogEM(x,K,iters,minVary,plotFlag,randconst,use_kmeans)
...
randConst = randconst;
p = randConst+rand(K,1); p = p/sum(p); %guesses probabilities of clusters
mn = mean(x,2); vr = std(x,[],2).^2; %mean and variance of the data

%-------------------- Modify the code here --------------------------------
% Change the random initialization with k-means algorithm, use 5
% iterations.
if(~use_kmeans)
    mu = mn*ones(1,K)+randn(N,K).*(sqrt(vr)/randConst*ones(1,K)); %mean of clusters
else
    mu = kmeans(x,K,5);
end
%------------------------------------------------------------------------
vary = vr*ones(1,K)*2; vary = (vary>=minVary).*vary + (vary<minVary)*minVary; %variance of clusters
...
for i=1:iters
  ...
  ivary = 1./vary;
  %compute log(p(i)*p(xn|i)) <- vector of length kxN (for each data point
  %and class)
  ...
  %find the class that maximizes posterior for each data point
...
%     i_sec = 2;
%     for k=1:K
%       plotEllipse(mu(i_first,k),mu(i_sec,k),vary(i_first,k),vary(i_sec,k),0,ellColor(mod(k, length(ellColor))+1));
%     end;
%     for t=1:T  plot(x(i_first,t),x(i_sec,t),char(plotString(mod(mxi(t), length(plotString))+1))); end
% 
%     axis equal;
%   end;
